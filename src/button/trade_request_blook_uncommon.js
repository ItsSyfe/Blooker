...
module.exports = {
	data: {
		customId: 'trade_request_blook_uncommon',
	},
	async execute(interaction) {
		const uncommonBlooks = await BlookHelper.getAllBlookNamesWithRarity('Uncommon');

		const rarityEmbeds = [];
		const selectMenus = [];
		let pagenumber = 0;

		for (let i = 0; i < Math.ceil(uncommonBlooks.length / 10); i++) {
			const blooks = uncommonBlooks.slice(i * 10, (i + 1) * 10);
			...
			if(i === 0) {
				selectMenu.addOptions({ label: `Any Uncommon Blook`, value: `uncommon` });
			}
