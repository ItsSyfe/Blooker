...
module.exports = {
	data: {
		customId: 'trade_request_blook_mystical',
	},
	async execute(interaction) {
		const mysticalBlooks = await BlookHelper.getAllBlookNamesWithRarity('Mystical');

		const rarityEmbeds = [];
		const selectMenus = [];
		let pagenumber = 0;

		for (let i = 0; i < Math.ceil(mysticalBlooks.length / 10); i++) {
			const blooks = mysticalBlooks.slice(i * 10, (i + 1) * 10);
			...
			if(i === 0) {
				selectMenu.addOptions({ label: `Any Mystical Blook`, value: `mystical` });
			}
