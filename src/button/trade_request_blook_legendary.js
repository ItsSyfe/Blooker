...
module.exports = {
	data: {
		customId: 'trade_request_blook_legendary',
	},
	async execute(interaction) {
		const legendaryBlooks = await BlookHelper.getAllBlookNamesWithRarity('Legendary');

		const rarityEmbeds = [];
		const selectMenus = [];
		let pagenumber = 0;

		for (let i = 0; i < Math.ceil(legendaryBlooks.length / 10); i++) {
			const blooks = legendaryBlooks.slice(i * 10, (i + 1) * 10);
			...
			if(i === 0) {
				selectMenu.addOptions({ label: `Any Legendary Blook`, value: `legendary` });
			}
