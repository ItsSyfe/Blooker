...
module.exports = {
	data: {
		customId: 'trade_request_blook_rare',
	},
	async execute(interaction) {
		const rareBlooks = await BlookHelper.getAllBlookNamesWithRarity('Rare');

		const rarityEmbeds = [];
		const selectMenus = [];
		let pagenumber = 0;

		for (let i = 0; i < Math.ceil(rareBlooks.length / 10); i++) {
			const blooks = rareBlooks.slice(i * 10, (i + 1) * 10);
			...
			if(i === 0) {
				selectMenu.addOptions({ label: `Any Rare Blook`, value: `rare` });
			}
